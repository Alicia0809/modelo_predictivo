# -*- coding: utf-8 -*-
"""
Sistema de Predicción de Riesgo Hídrico
...
def cargar_recursos():
    ...
    - Variables: variables.csv históricas utilizadas en el entrenamiento
    """
...
st.sidebar.success("✅ Modelo (RandomForest) + GEE listos")
...
def predecir_historico(anio, mes):
    ...
    if fila.empty:
        raise Exception("No existen datos históricos.")

    X = construir_features_desde_dataframe(fila)

    pred = modelo.predict(X)[0]

    prob = modelo.predict_proba(X)[0]

    return pred, prob, fila

def predecir_futuro(anio, mes):

    df = obtener_variables_gee(anio, mes)
...
try:
    pred, prob, datos_row = None, None, None # Initialize to avoid NameError if an error occurs before assignment

    if modo == "historico":
        pred, prob, datos_row = predecir_historico(anio_sel, mes_sel)

    elif modo == "futuro":
        pred, prob, datos_row = predecir_futuro(anio_sel, mes_sel)

    else: # modo == "mes_siguiente"
        pred, prob, datos_row = predecir_mes_siguiente()

    # Check if prediction was successful before displaying results
    if pred is not None and prob is not None and datos_row is not None:
        mostrar_resultado(pred, prob, datos_row)
